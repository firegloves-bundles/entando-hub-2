...
public class LoggingAspect {
	...
	public void logAfterget(JoinPoint joinPoint) {
		if (log.isDebugEnabled()) {
			...
			log.debug("Audit_Logging: For " + request.getMethod() + " method {}.{}() by user: " + auth.getName()+ " at time:" + timeStamp,
					joinPoint.getSignature().getDeclaringTypeName(), joinPoint.getSignature().getName(), auth.getName(),auth.getPrincipal(), timeStamp, Arrays.toString(joinPoint.getArgs()));
		}
	}
	...
	public void logAfterallOtherMethods(JoinPoint joinPoint) {
		if (log.isInfoEnabled()) {
			...
			String timeStamp = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(Calendar.getInstance().getTime());
			HttpServletRequest request = ((ServletRequestAttributes) RequestContextHolder.getRequestAttributes()).getRequest();
			log.info("Audit_Logging: For " + request.getMethod() + " method {}.{}() by user: " + auth.getName()+ " at time:" + timeStamp,
					joinPoint.getSignature().getDeclaringTypeName(), joinPoint.getSignature().getName(), auth.getName(),auth.getPrincipal(), timeStamp, Arrays.toString(joinPoint.getArgs()));
