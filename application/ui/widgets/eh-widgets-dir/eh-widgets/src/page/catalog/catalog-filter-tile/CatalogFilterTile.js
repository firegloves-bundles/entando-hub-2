...
const CatalogFilterTile = ({categories, onFilterChange}) => {
  ...
  const onChange = (value, id, event) => {
    if (id === "-1") {
      //all clicked
      Object.keys(checkboxStatuses).forEach(
          (key) => checkboxStatuses[key] = (value === true))
    } else {
    ...
    onFilterChange(Object.keys(checkboxStatuses).filter(
        key => (key !== "-1" && checkboxStatuses[key])))
  }
  const prefix = "catalog-filter";
  const listItems = categories.map(
      (category) => <Checkbox checked={checkboxStatuses[category.categoryId]}
                              onChange={onChange} key={category.categoryId}
                              labelText={category.name + " - "
                              + category.categoryId} id={category.categoryId}/>)
  return (
      <Tile>
        <fieldset className={`${prefix}--fieldset`}>
          <Checkbox checked={checkboxStatuses["-1"]} onChange={onChange}
                    key={"-1"} labelText={"All"}
                    id={"-1"}/>
          {listItems}
        </fieldset>
